...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(trie, board, node, posR, posC, visit, valWords):
            ...
            visit.add((posR, posC))
            dfs(trie, board, node, posR + 1, posC, visit, valWords)
            dfs(trie, board, node, posR - 1, posC, visit, valWords)
            dfs(trie, board, node, posR, posC + 1, visit, valWords)
            dfs(trie, board, node, posR, posC - 1, visit, valWords)
        ...
        for r in range(len(board)):
            for c in range(len(board[0])):
                dfs(trie, board, root, r, c, visit, valWords)
